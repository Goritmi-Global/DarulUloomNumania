<template>
    ...
    <main id="main" class="main">
        ...
        <div class="pagetitle d-flex justify-content-between align-items-start mb-3">
            <div>
                <h1 class="theme-text-color mb-1">{{ translate("Teacher Details") }}</h1>
                <nav>
                    <ol class="breadcrumb">
                        <li class="breadcrumb-item">
                            <Link href="/dashboard">{{ translate("Darul Oloom") }}</Link>
                        </li>
                        <li class="breadcrumb-item active">{{ record[0].name }}</li>
                    </ol>
                </nav>
            </div>
        </div>
                    ...
                    <div class="card border-0 shadow-lg rounded-4 h-100 overflow-hidden">
                        ...
                        <div class="card-body">
                            ...
                            <div>
                                ...
                                <div class="fw-semibold">{{ record[0].name }}</div>
                                <div class="small text-muted mt-1">
                                    {{ record[0].designation ?? '-' }} · {{ record[0].contact ?? '-' }}
                                </div>
                            </div>
                        </div>
                    </div>
                            ...
                            <div class="metric-tile highlight">
                                <div class="d-flex justify-content-between align-items-center">
                                    ...
                                    <span class="badge rounded-pill"
                                        :class="totals.net >= 0 ? 'bg-success' : 'bg-danger'">
                ...
                <div class="card-body">
                    ...
                    <div class="table-responsive">
                        <table class="table table-hover align-middle m-0">
                            ...
                            <tbody>
                                <tr v-for="(t, i) in record" :key="t.id">
                                    <td class="text-muted">{{ indexBase + i + 1 }}</td>
                                    <td>{{ t.name }}</td>
                                    <td>{{ t.designation ?? '-' }}</td>
                                    <td>{{ t.monthly_salary ?? '-' }}</td>
                                    <td>{{ t.contact ?? '-' }}</td>
                                </tr>

                                <tr v-if="!record.length">
