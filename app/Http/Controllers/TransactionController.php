...
class TransactionController extends Controller
{
    ...
    public function storeMultiple(Request $request)
    {
        ...
        $request->validate([
            ...
            'transactions.*.cash_in' => 'nullable|numeric',
            'transactions.*.date' => 'required|date',
            'transactions.*.remarks' => 'nullable|string|max:255',
            'transactions.*.process_type' => 'required|string',
        ]);

        try {
            foreach ($request->transactions as $transaction) {
                $model = new Transaction;
                $model->cash_in = $transaction['cash_in'] ?? null;
                $model->cash_out = $transaction['cash_out'] ?? null;
                $model->transaction_date = $transaction['date'] ?? null;
                $model->islamic_date = $transaction['islamic_date'] ?? null;
                $model->ref_no = $transaction['ref_no'] ?? null;
                $model->method = $transaction['method'] ?? null;
                $model->remarks = $transaction['remarks'] ?? null;
                $model->user_id = auth()->user()->id;
                $model->transaction_type = $transaction['expense_type'] ?? null;
                $model->received_from = $transaction['received_from'] ?? null;
                $model->received_by = $transaction['received_by'] ?? null;

                // If you handle receipt_image as file upload, adjust here
                if (isset($transaction['receipt_image']) && $transaction['receipt_image']) {
                    $model->receipt_image = $transaction['receipt_image'];
                }
            ...
            return response()->json([
                'success' => true,
                'message' => 'Multiple transactions saved successfully.',
            ], 201);
